...
def render_funnel(df_period: pd.DataFrame, df_full: pd.DataFrame) -> None:
    st.subheader("🔽 Embudo Asignados → Calificados → Cierres")

    data = funnel_by_advisor(df_period, df_full)

    if data.empty:
        st.info("Sin asesores con cierres en este período.")
        return
